...
// Acceso consistente a la experiencia completa de VALL AI desde todas las
// secciones. Se inyecta aquí para no duplicar la navegación en cada página.
(function () {
    function initVallAiNav() {
        var center = document.querySelector('.hdr-nav .nav-center');
        if (!center || center.querySelector('a[href="/pages/vall-ai.html"]')) return;
        var link = document.createElement('a');
        link.className = 'nav-link' + (/\/pages\/vall-ai\.html$/i.test(location.pathname) ? ' active' : '');
        link.href = '/pages/vall-ai.html';
        link.innerHTML = '<i class="fas fa-wand-magic-sparkles"></i> VALL AI';
        link.addEventListener('click', function () {
            var nav = document.querySelector('.hdr-nav');
            var button = document.querySelector('.vn-hamburger');
            if (nav) nav.classList.remove('vn-nav-open');
            if (button) button.classList.remove('vn-ham-open');
            document.body.style.overflow = '';
        });
        center.appendChild(link);
    }
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initVallAiNav);
    else initVallAiNav();
})();
