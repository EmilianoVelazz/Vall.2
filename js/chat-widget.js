...
11. GRÁFICAS — REGLA OBLIGATORIA: si el usuario pide una gráfica/gráfico/chart/
    ...
    async function _geminiStreamRequest(prompt, tier, onChunk) {
        ...
        try {
            ...
            const res = await fetch(API_BASE + '/api/ai-insight-stream', {
                ...
                body: JSON.stringify({ prompt, systemPrompt: SYSTEM_PROMPT, tier }),
                signal: ctrl.signal,
            });
            if (!res.ok || !res.body) {
            ...
            while (true) {
                ...
                while ((idx = buf.indexOf('\n\n')) !== -1) {
                    ...
                    if (payload.done) { if (payload.tier) usedTier = payload.tier; return { text: full, tier: usedTier }; }
                }
            }
            return { text: full, tier: usedTier };
        } finally {
            clearTimeout(tid);
    ...
    async function callGemini(userMessage, onChunk, tier = 'flash') {
        ...
        if (history.length > 0) {
            ...
            if (recent.length) {
                ...
                recent.forEach(h => {
                    parts.push(`${h.role === 'user' ? 'Usuario' : 'VALL-AI'}: ${h.text.slice(0, 500)}`);
                });
                parts.push('[FIN CONVERSACIÓN]\n');
            }
        }
        ...
        const { text: reply, tier: usedTier } = await _geminiStreamRequest(parts.join('\n'), tier, onChunk);
...
<button id="vn-cw-btn" title="Hablar con VALL-AI" aria-label="Abrir asistente VALL-AI">
    ...
    <img id="vn-cw-img-hover"  src="${mascotaFull}" alt="VALL-AI">
        ...
        function addMsg(content, role, ts) {
            const row = document.createElement('div');
            row.className = `vn-m ${role}`;
            const bbl = document.createElement('div');
            bbl.className = 'vn-bbl';
            if (role === 'b') { bbl.innerHTML = renderMd(content); _hydrateCharts(bbl); }
        ...
        function _attachActions(row, bbl, content, { forceDownload = false, regenerable = false, ts = Date.now(), tier = null } = {}) {
            ...
            if ((content.length > 150 || forceDownload) && typeof VDS !== 'undefined' && VDS.downloadAsWord) {
                ...
                dl.addEventListener('click', () => {
                    ...
                    VDS.downloadAsWord(`VALL-AI_reporte_${stamp.replace(/\//g, '-')}`, content, { title });
                });
                bar.appendChild(dl);
            }
        ...
        function addStreamingMsg() {
            ...
            return {
                ...
                finish(finalText, opts) { bbl.innerHTML = renderMd(finalText); _attachActions(row, bbl, finalText, opts); _hydrateCharts(bbl); _scrollToBottom(false); },
        ...
        function _renderActiveConversation() {
            ...
            history.forEach(h => {
                if (h._hidden) return;
                const row = addMsg(h.text, h.role === 'user' ? 'u' : 'b', h.ts);
        ...
        async function ask(text, { regenerateRow, regenerateBbl } = {}) {
            ...
            const streamMsg = (regenerateRow && regenerateBbl)
                ? { row: regenerateRow, bbl: regenerateBbl,
                    ...
                    finish(finalText, opts) { regenerateBbl.innerHTML = renderMd(finalText); _attachActions(regenerateRow, regenerateBbl, finalText, opts); _hydrateCharts(regenerateBbl); _scrollToBottom(false); },
            ...
            while (true) {
                try {
                    const { reply, tier: usedTier } = await callGemini(text, (partial) => {
                        lastPartial = partial;
                        if (firstChunk) { hideTyping(); firstChunk = false; }
                        streamMsg.update(partial);
                    }, tier);
                    hideTyping();
                    streamMsg.finish(reply, { forceDownload: wantsDoc, regenerable: true, tier: usedTier });
                    updateBadge();
